  ...
  const NetworkGraph = ({ data, onNodeSelect, onEdgeSelect }) => {
    ...
    const processGraphData = useCallback((rawData) => {
      ...
      const nodes = rawData.nodes.map(node => {
        ...
        const nodeType = node.attributes.label;
        ...
        const originalLabel = node.attributes.name || 'Unknown';
        ...
        return {
          ...
          heightConstraint: {
            maximum: 60 // Maximum height in pixels
          },
          shapeProperties: {
            borderDashes: false,
            borderRadius: 5,
    ...
    useEffect(() => {
      ...
      networkRef.current.on('click', function(params) {
        if (params.nodes.length > 0) {
          ...
          const node = data.nodes.find(n => n.key === nodeId);
          if (node) {
            onNodeSelect(node);
          }
          ...
          const edge = data.edges.find(e => e.key === edgeId);
          if (edge) {
            onEdgeSelect(edge);
          }
        } else {
