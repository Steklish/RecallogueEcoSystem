...
class Generator:
    ...
    def __init__(self, base: str):
        ...
        if os.getenv("USE_GEMINI") == '1':
            print(f"{INFO_COLOR}Using GEMINI model as LLM backend{RESET}")
            ...
            print(f"{INFO_COLOR}Using QWEN model as LLM backend{RESET}")
            ...
            print(f"{INFO_COLOR}Using local Llama server as LLM backend{RESET}")
            self.llama_client = LlamaGenAI(base)
            self.complete_funtion = self.llama_client.complete
            self._backend_type = f"local <{self.base}>"
        
        print(f"{SUCCESS_COLOR}Generator instantiated successfully.{RESET}")
...
ai_generator = Generator(base=settings.LLAMACPP_CHAT_BASE)